...
data <- read_excel("./pre_post_carriage_GPSCs.xlsx")


# Get reference counts for serotype 6A
ref <- data %>% filter(GPSC == "233")
ref_pre <- ref$Pre
ref_post <- ref$Post
...
results <- read_excel("./233reference_carriage_>4.xlsx")
...
forest_plot <- ggplot(significant_before_correction, aes(x = odds_ratio, y = GPSC)) +
  ...
  labs(
    x = "Odds Ratio (log scale)",
    y = "GPSC",
    title = "Changes Post-PCV10 in Reference to GPSC233"
  ) +
  theme_minimal() +
...
aboveFour <- names(counts[counts > 4])
...
gpsc_order <- c(1054,864,21,170,1053,862,1051,879,54,67,884,181,9,2,62,251,217,88,53,61,883,661,233)
...
large_plot <- ( forest_plot2/GPSC_Plot) + plot_layout(heights = c(2,4)) 
